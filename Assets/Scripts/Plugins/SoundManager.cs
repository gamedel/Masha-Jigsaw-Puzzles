...
namespace EazyTools.SoundManager
{
    public class SoundManager : MonoBehaviour
    {
        ...
        public static bool isSoundPlaying(AudioClip clip)
        {

            if (GameObject.Find("EazySoundManager")!=null && GameObject.Find("EazySoundManager").GetComponent<AudioSource>() != null)
            {
                List<int> keys;
                // Update sound fx
                keys = new List<int>(soundsAudio.Keys);

                foreach (AudioSource comp in GameObject.Find("EazySoundManager").GetComponents<AudioSource>())
                {
                    if (comp.clip == clip)
                    {
                        return true;
        ...
        {
            return PlaySound(clip, 1f, false, null,false);
        }
        ...
        {
            return PlaySound(clip, volume, false, null,false);
        }
        ...
        {
            return PlaySound(clip, 1f, loop, null,false);
        }
        public static int PlaySound(AudioClip clip, float volume, bool persist)
        {
            return PlaySound(clip, volume, false, null, persist);
        }
        ...
        /// <returns>The ID of the created Audio object</returns>
        public static int PlaySound(AudioClip clip, float volume, bool loop, Transform sourceTransform,bool persist)
        {
            if (clip == null)
            {
               // Debug.LogError("Sound Manager: Audio clip is null, cannot play music", clip);
            ...
            // Create the audioSource
            AudioSource audioSource = instance.gameObject.AddComponent<AudioSource>() as AudioSource;
            Audio audio = new Audio(Audio.AudioType.Sound, clip, loop, persist, volume, 0f, 0f, sourceTransform);
